...
void histograma(char *filename, int tramos, char *destino){
	...
	//Se aplica la funcion de escala de grises y se guardan los valores en una matriz, paralelizando el for para conserguir que las iteraciones del bucle sean realizadas por todos los hilos del procesador
	...
	for(int i=0; i< tramos; i++){
        	arrayTramos[i] = 0;
		//Se paraleliza el bucle para que las iteraciones se repartan entre los diferentes hilos del procesador forzando la actualización atómica de una posición de memoria en arrayTramos
...
void maximoMinimo(char *filename, char *destino){
	...
	//Se hallan los valores de maximo y minimo para las matrices de colores paralelizando el for para conserguir que las iteraciones del bucle sean realizadas por todos los hilos del procesador
	#pragma omp parallel for schedule(guided)
	for(unsigned int i = 0; i< longitud; i++){
...
void mascara(char *filename, char *rutaMascara, char *destino){
	...
	if(alturaM != altura && anchuraM != anchura){
		...
		//Se aplica la mascara paralelizando el for para conserguir que las iteraciones del bucle sean realizadas por todos los hilos del procesador
		#pragma omp parallel for 
		for(unsigned int i = 0; i< longitud; i++){
...
void rotacion(char *filename, double grados ,char *destino){
	...
	unsigned char** rojoR=new unsigned char*[anchura];
	for (unsigned int i = 0; i < anchura; i++){
		rojoR[i]= new unsigned char[altura];
	}

	unsigned char** verdeR=new unsigned char*[anchura];
	for (unsigned int i = 0; i < anchura; i++){
		verdeR[i]= new unsigned char[altura];
	}
	
	unsigned char** azulR=new unsigned char*[anchura];
	for (unsigned int i = 0; i < anchura; i++){
		azulR[i]= new unsigned char[altura];
	}
		...
		for(unsigned int j = 0; j< anchura; j++){
			...
			//Si la posicion entra en el rango se aplica la rotacion
            		if(xf >= 0 && xf < anchura && yf >= 0 && yf < altura ){	
            			rojoR[yf][xf] = rojo[j][i];
            			verdeR[yf][xf] = verde[j][i];
            			azulR[yf][xf] = azul[j][i];            
            		}
			else{
				rojoR[i][j] = 0;
        	    		verdeR[i][j] = 0;
	...
	for(unsigned int i = 0; i< altura; i++){
		...
		fwrite (&rojoR[i][0] , sizeof(unsigned char), total, escribir);
	  
	}
	for(unsigned int i = 0; i< altura; i++){
		
		fwrite (&verdeR[i][0] , sizeof(unsigned char), total, escribir);
	  	
	}
	for(unsigned int i = 0; i< altura; i++){

		fwrite (&azulR[i][0] , sizeof(unsigned char), total, escribir);
